...
contract Sales {
    ...
    function addFact(bytes32 OnlineSalesKey,uint SalesQuantity,bytes32 SalesAmount,bytes32 TotalCost,bytes32 UnitCost,bytes32 UnitPrice) public returns (bytes32 ,uint ,bytes32 ,bytes32 ,bytes32 ,bytes32, uint ID){
        facts[dataId].OnlineSalesKey= OnlineSalesKey;
        facts[dataId].SalesQuantity= SalesQuantity;
        facts[dataId].SalesAmount= SalesAmount;
        facts[dataId].TotalCost= TotalCost;
        facts[dataId].UnitCost= UnitCost;
        facts[dataId].UnitPrice= UnitPrice;
        facts[dataId].timestamp = now;
        return (facts[dataId].OnlineSalesKey,facts[dataId].SalesQuantity,facts[dataId].SalesAmount,facts[dataId].TotalCost,facts[dataId].UnitCost,facts[dataId].UnitPrice,dataId);
    }


    function getFact(uint id) public constant returns (bytes32 OnlineSalesKey,uint SalesQuantity,bytes32 SalesAmount,bytes32 TotalCost,bytes32 UnitCost,bytes32 UnitPrice){
