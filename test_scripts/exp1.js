...
const keepAliveAgent = new http.Agent({ keepAlive: true, timeout: 1500000000 });
const keepAliveAgent2 = new http.Agent({ keepAlive: true, timeout: 1500000000 });
...
fs.writeFile('test_data/' + fileToSaveTestData + '1' + '.json', JSON.stringify(all), function (err) {
    ...
    fs.writeFile('test_data/' + fileToSaveTestData + '2' + '.json', JSON.stringify(allSecondRun), function (err) {
        ...
        fs.writeFile('test_data/' + fileToSaveTestData + '3' + '.json', JSON.stringify(allThirdRun), function (err) {
            ...
            http.get({host: 'localhost', path: '/load_dataset/' + fileToSaveTestData + '1' + '.json', port: 3000, agent: keepAliveAgent, headers:{
                ...
                res.on("end", () => {
                    ...
                    http.get({host: 'localhost', path: '/getViewByName/A|B(COUNT)', port: 3000, agent: keepAliveAgent2, headers:{
                            'Connection': 'keep-alive'
                        }}, resp => {
                        let gbBody = "";
                        resp.on("data", data => {
                        ...
                        resp.on("end", () => {
                            let JSONresp = JSON.parse(gbBody);
           ...
           // http.get({method: 'GET', host: 'localhost', path: 'load_dataset/' + fileToSaveTestData + '1' + '.json', port: 3000, agent: keepAliveAgent, headers: {
